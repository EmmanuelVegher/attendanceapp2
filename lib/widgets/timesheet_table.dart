...
class _TimesheetTableState extends State<TimesheetTable> {
  final DateTime startDate = DateTime.now().subtract(Duration(days: 10)); // Start from 20th of previous month
  final DateTime endDate = DateTime.now().add(Duration(days: 10)); // End on 19th of current month
  List<DateTime> dates = [];
  ...
  Widget build(BuildContext context) {
    return Table(
      ...
      columnWidths: {0: FixedColumnWidth(120)}, // Fix width for the Project column
      children: [
        _buildHeaderRow(),
        _buildDateRow(),
        ..._buildHourRows(),
  ...
  TableRow _buildHeaderRow() {
    return TableRow(
      children: [
        TableCell(child: Text('Project', style: TextStyle(fontWeight: FontWeight.bold))),
  ...
  List<TableRow> _buildHourRows() {
    return [
      TableRow(
        children: [
          TableCell(child: Text('Project from Bio', textAlign: TextAlign.center)),
          ...dates.map((date) => TableCell(
